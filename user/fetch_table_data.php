...
if ($table === 'All fields') {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_array()) {
            ...
            $sql = "SELECT title, author, Date_Of_Publication_Copyright, record_cover, No_Of_Copies FROM `$tableName`"; // Include 'No_Of_Copies'
            ...
            if ($tableResult->num_rows > 0) {
                while ($tableRow = $tableResult->fetch_assoc()) {
                    ...
                    $allData[] = [
                        ...
                        'copies' => $tableRow['No_Of_Copies'] // Add the number of copies to the response
                    ];
                }
            }
        }
    ...
    $sql = "SELECT title, author, Date_Of_Publication_Copyright, record_cover, No_Of_Copies FROM `$table`"; // Include 'No_Of_Copies'
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $data[] = [
                ...
                'table' => $table,
                'coverImage' => 'data:image/jpeg;base64,' . $coverImageBase64, // Prefix with MIME type
                'copies' => $row['No_Of_Copies'] // Add the number of copies to the response
            ];
        }
    }
