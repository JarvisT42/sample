...
if ($table === 'All fields') {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_array()) {
            ...
            $sql = "SELECT id, title, author, Date_Of_Publication_Copyright, record_cover, No_Of_Copies, status FROM `$tableName` WHERE status = 'damage'";
    ...
    $sql = "SELECT id, title, author, Date_Of_Publication_Copyright, record_cover, No_Of_Copies, status FROM `$table` where status = 'damage'";
